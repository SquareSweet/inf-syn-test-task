...
public class AccountServiceImpl implements AccountService {
    ...
    public BalanceResponse checkBalance(String username) {
        Account account = accountDao.getByUsername(username).orElseThrow(
                () -> {
                    log.error("User {} not found or does not have account", username);
                    return new UserNotFoundException("User " + username + " not found or does not have an account");
                }
        );
    ...
    public TransferResponse transferToUsername(String senderUsername, TransferRequest transferRequest) {
        ...
        Account senderAccount = accountDao.getByUsername(senderUsername).orElseThrow(
                () -> {
                    log.error("User {} not found or does not have account", senderUsername);
                    return new UserNotFoundException("User " + senderUsername + " not found or does not have an account");
                }
        );
        ...
        if (senderAccount.getBalance() < amountLong) {
            log.error("User {} has insufficient balance", senderUsername);
        ...
        Account receiverAccount = accountDao.getByUsername(transferRequest.getReceiverUsername()).orElseThrow(
                () -> {
                    log.error("User {} not found or does not have account", transferRequest.getReceiverUsername());
                    return new UserNotFoundException("User " + transferRequest.getReceiverUsername() +
